# Session 2 — Sample Answer Guidance

Use these notes when reviewing student work on the Friction Map and Debt Inventory exercise.

## Strong Day 2 friction maps usually include
...
- **Evidence that students can separate symptoms from root causes.** "Tests are flaky" is a symptom; the root cause might be environment debt (shared test database) or data debt (no stable seed fixtures).
- **At least one non-code debt item in the top priorities.** A top-three that is all code debt is a warning sign — the student may still be equating debt with messy code. Strong answers include environment, release, observability, workflow, or knowledge debt in the top tier.
...
These show up across the week, not just on Day 2, but they are especially common in Day 2 work.

- **Treating architecture as optional.** Students skip design debt because it feels abstract. Push them to name a specific case where unclear boundaries slowed a recent change.
...
- **Proposing big rewrites instead of staged improvements.** If a student's top-three includes "rewrite the monolith," ask for a three-step version of that plan that could ship in 30 days.
...
- Can the student separate this item's symptom from its root cause?
