# Workshop 3 — Instructor Notes

## The main move of this workshop

Shift the room from "AI is impressive" to "AI is useful when aimed carefully." The workshop works when participants leave with a sharper sense of **which AI tasks they will trust, which they will review, and which they will refuse** — and with a prompt shape they can actually reuse.

## Misconceptions most worth correcting today

### Misconception 1 — AI matters mainly because it writes code quickly

**Correction.** AI often creates more value when used for codebase understanding, documentation, test scaffolding, refactoring prep, and workflow cleanup. The capability map on Slide 2 (inspect / explain / propose / scaffold / standardize) is the spine of the workshop. Return to it whenever a participant frames AI primarily as a typing accelerator.
...
## Places participants may be skeptical
...
- Opening move: show the weak prompt on Slide 3 live, then the tightened one. Let the room feel the difference before you explain it.
- Slides 2 (capability map) and 3 (prompt shape). Keep conceptual framing concise.
- Slides 4 (docs/architecture) and 5 (tests/small refactors). Move quickly — these are setup for the exercises.
- Slides 6 (failure modes) and 7 (review rules). This is where engineering discipline lands. Do not shortcut.
- Choose **one** core exercise to run live in the room (Documentation Gap Hunt is the lowest-barrier; Architecture Map is the highest-value). Assign the other two as follow-up on participants' own repos. The lightweight exercises can be sprinkled as energy drops.
- Exit prompt and debrief.
...
- **Small, scoped, reviewable.** If a participant's AI output cannot be reviewed line by line, the scope is wrong.
