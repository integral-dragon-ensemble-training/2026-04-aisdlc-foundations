...
Workshop 3 pushes participants from "AI is impressive" to "AI is useful when aimed carefully." The three core exercises each produce an inspectable artifact against a real repository. The three lightweight exercises sharpen prompt and task judgment.

Pair these exercises with the **Participant Prompt Template for Codebase Analysis** (see `workshop-3-prompt-template.md`).

Framing lines for the workshop:
...
AI can generate candidate cases and scaffolds, but participants must filter aggressively. Reject tests that assert implementation details, over-mock, or merely exercise lines without testing behavior.
