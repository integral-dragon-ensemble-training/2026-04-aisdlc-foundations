# Workshop 2 — Instructor Notes

## Workshop purpose

Move participants from "debt equals messy code" to "debt is the repeated tax the system charges on every change." By the end of the workshop they should be able to classify a debt item precisely, estimate its interest, and prioritize it by impact on flow.

## Opening move

Ask: *"What do engineers complain about repeatedly that is not itself the business feature they are trying to change?"*

Let participants answer for two or three minutes before introducing the taxonomy. The complaints are the raw material for the exercise; do not let the lecture replace their examples.
...
*Correction:* Technical debt also lives in documentation, environment setup, test data, release process, observability, and security hygiene. Push back explicitly when participants reduce it to code formatting.

### Misconception 2 — AI matters mainly because it writes code quickly

*Correction:* In this workshop, AI's value is inspection and classification — turning a repo tour into a categorized debt inventory, clustering findings, and drafting a first-pass backlog. AI is not deciding tradeoffs.
...
## Where participants are often skeptical
...
Open with the framing question, define debt operationally, and introduce principal vs. interest. Walk the nine-category taxonomy with concrete examples, using participant complaints from the opening. Cover "How teams pay interest" and the priority 2x2 framing, reinforcing that high-frequency pain beats elegance. Run the Friction Map exercise in groups; if there is room, insert "Smell or Debt?" as a warm-up. Close with a debrief — share one group's top three and compare rankings — then the exit prompt and bridge to the next workshop.
...
Ask participants to write one sentence before leaving. These answers are the input for the next workshop — they tell you where AI-assisted inspection should point first.
...
## What a "ready for the next workshop" participant looks like
