...
Two core exercises plus three lightweight in-class activities. Pace them as the session allows; the core exercises are the backbone, the lightweight ones are warm-ups and fillers.
...
AI can propose missing telemetry and draft logging standards or runbook snippets. Students evaluate which suggestions carry real signal and which are noise.
...
- The AI-Assisted Refactoring Review Checklist (see `session-4-rubric.md`).
...
Use AI to summarize the change and enumerate risks, but students make the final decision. Do not let fluent summaries substitute for judgment.
...
Give students a small request flow (for example: an API endpoint that receives a payment request, calls a fraud-check service, writes to a database, and returns a receipt). Ask:
...
Show the class a hypothetical AI-generated PR summary. Ask students to rate it on a 1–5 scale in each of:
